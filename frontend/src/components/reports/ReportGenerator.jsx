    ...
    <div className="fixed inset-0 bg-[#1B1B1E]/50 backdrop-blur-sm overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full mx-4 p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleGenerate} className="space-y-4">
          {/* Info Card */}
          <div className="p-4 bg-[#F7F6F2] border border-[#EFEDE8] rounded-xl">
            <div className="flex items-start gap-3">
              <div className="p-2 bg-[#E8F5E9] rounded-lg flex-shrink-0">
                <FiDownload size={18} className="text-[#2E7D32]" />
              </div>
              <div className="flex-1">
                <h4 className="text-sm font-semibold text-[#1B1B1E] pj-body mb-1">
                  Excel Report (.xlsx)
                </h4>
                <p className="text-xs text-[#8A8985] pj-body leading-relaxed">
                  Generates a two-sheet workbook:
                </p>
                <ul className="mt-2 space-y-1 text-xs text-[#5B5A56] pj-body">
                  <li className="flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#3E3AA0]"></span>
                    <span><strong>Dashboard</strong> — KPIs, progress, priority breakdown, team workload & milestones</span>
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#3E3AA0]"></span>
                    <span><strong>Tracking Sheet</strong> — Full task list with status, priority & % complete</span>
                  </li>
                </ul>
              </div>
            </div>
