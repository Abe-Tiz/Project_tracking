...
def create_app():
    ...
    def get_user_identity():
        """Extract user identity from JWT token if available"""
        try:
    ...
    def log_audit(action, method, endpoint, user_id=None, request_data=None, response_status=None, error=None):
        ...
        logger.info(f"📋 AUDIT: {action} | User: {user_id} | {method} {endpoint} | Status: {response_status}")
    ...
    def audit_before_request():
        ...
        if request.method in ['POST', 'PUT', 'PATCH', 'DELETE']:
            ...
            logger.info(f"🔐 AUDIT: {request.method} {request.path} initiated by user: {user_id}")
    ...
    def audit_after_request(response):
        ...
        if request.method in ['POST', 'PUT', 'PATCH', 'DELETE']:
            ...
            if hasattr(request, 'audit_context'):
                user_id = request.audit_context.get('user_id')
                request_data = request.audit_context.get('request_data')
    ...
    def audit_error_handler(error):
        ...
        log_audit(
            action=f"{request.method} {request.path} - ERROR",
            method=request.method,
            endpoint=request.path,
            user_id=user_id,
            request_data=request_data,
        ...
        # Re-raise the error to be handled by other error handlers
        raise error
    ...
    # Request logging middleware (simplified - audit logging handles this now)
    @app.before_request
    def log_request_info():
        """Log all incoming requests"""
        if request.method != 'OPTIONS' and not request.path.startswith('/uploads/'):
            logger.info(f"📥 {request.method} {request.path}")
    ...
    # Import and register routes (after audit logging is set up)
    from routes.auth_routes import auth_bp
    from routes.project_routes import project_bp
    from routes.task_routes import task_bp
    from routes.report_routes import report_bp
